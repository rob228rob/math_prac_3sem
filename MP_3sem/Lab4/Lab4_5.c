...
int Stack_char_push(Stack_char ** head, char data)
{
...
int Stack_char_pop(Stack_char ** head, char * num)
{
...
int Stack_number_push(Stack_number ** head, long long data)
{
...
int Stack_number_pop(Stack_number ** head, long long * num)
{
...
void response(FILE * output, enum errors err, int count_line)
{
    switch (err)
    {
        case INVALID_INPUT:
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        while (!feof(input))
        {
            ...
            else if(err != OK)
            {
                ...
                response(err_output, err, count_line);
                ++count_line;
                free(err_filename);
                continue;
            }
